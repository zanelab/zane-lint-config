...
export class LintStagedAnswer extends AbstractAnswer {
  ...
  installer = new Installer(['lint-staged', '@zanelab/lint-staged'])

  config = new Config([
    new CosmiConfigOption({
      configFileName: 'lint-staged.config.js',
      esmImportConfigContent: 'import lintStaged from "@zanelab/lint-staged"',
      esmExportConfigContent: 'lintStaged',
      cjsImportConfigContent: 'const lintStaged = require("@zanelab/lint-staged")',
